...
import scala.meta.metals.providers.RenameProvider._
...
object RenameProviderTest extends MegaSuite with LazyLogging {
    ...
    "rename" - {
      "first test" - {
        val params = RenameParams(
          TextDocumentIdentifier(path.UserTestUri.value),
          Position(3, 7),
          "renamedUser"
        )
        val edits = rename(params, index)
        println(edits)
        true
      }
    }
